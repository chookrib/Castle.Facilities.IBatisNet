...
namespace IBatisNet.DataMapper.Configuration
{
    ...
    public class DomSqlMapBuilder
    {
        ...
        private void Initialize()
        {
            ...
            //=================================================================
            //为sqlMaps添加include支持，支持包含文件
            foreach (XmlNode node in _configScope.SqlMapConfigDocument.SelectNodes(ApplyDataMapperNamespacePrefix("sqlMapConfig/sqlMaps/include"), _configScope.XmlNamespaceManager))
            {
                XmlDocument includeDocument = Resources.GetAsXmlDocument(node, _configScope.Properties);

                //foreach (XmlNode n in includeDocument.SelectNodes(ApplySqlMapNamespacePrefix("sqlMaps/sqlMap"), _configScope.XmlNamespaceManager))
                //{
                //	_configScope.NodeContext = n;
                //	ConfigureSqlMap();
                //}

                foreach (XmlNode n in includeDocument.SelectNodes("/*/sqlMap"))
                {
                    _configScope.NodeContext = n;
                    ConfigureSqlMap();
                }
